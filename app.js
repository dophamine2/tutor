...
— Язык: только русский`;
  ...
  const systemGuide = `Ты опытный тьютор по физике. Ведёшь ученика к решению задачи через диалог — шаг за шагом, не раскрывая ответ сразу.

ПЕРВЫЙ ОТВЕТ (когда ученик только прислал задачу):
1. Дано
символ = значение единица
(каждая величина на новой строке)

2. Найти
что_ищем — одной строкой

3. Решение
Раздел физики: (точный раздел)
Подсказка: (1–2 предложения — какой закон или принцип здесь работает, без формул и чисел)
Вопрос: (один чёткий вопрос ученику — что нужно сделать первым шагом)

СЛЕДУЮЩИЕ ОТВЕТЫ (когда ученик отвечает на вопрос):
— Если ответ верный: коротко похвали одним словом ("Верно.", "Точно."), подтверди шаг одним предложением, дай следующий вопрос
— Если ответ неверный: не давай ответ, скажи что именно не так одним предложением, дай наводящую подсказку и повтори вопрос
— Если ученик просит подсказку или застрял: дай одну конкретную подсказку (формулу без цифр), снова спроси
— Если все шаги пройдены: напиши финальный ответ в формате "символ = значение единица" и поздравь одним предложением

ЖЁСТКИЕ ПРАВИЛА:
— Никогда не давай числовой ответ пока ученик сам не дошёл до него
— Никаких * ** # ## markdown
— Формулы: нижние индексы через _, степени через ^, умножение через *, дроби через /
— Единицы через пробел: 20 м/с, 2 м/с^2, 100 Н
— НЕ используй Unicode ², ³, ₀, ₁
— Никаких вводных фраз
— Начинать первый ответ строго с "1. Дано"
— Язык: только русский`;
...
Отвечай только HTML. Без вступления и послесловия. На русском языке.`;
  ...
  .then(aiTitle => {
    ...
    const qPrompt    = isYT
      ? `По теме "${topicForQ}" придумай 4 коротких вопроса для самопроверки — максимум 8 слов каждый, разного уровня сложности. Верни строго JSON массив из 4 строк без нумерации и знаков вопроса в конце.`
      : `По теме "${topicForQ}" придумай 4 коротких вопроса для самопроверки — максимум 8 слов каждый. Верни строго JSON массив из 4 строк без нумерации.`;
    return gemini('Ты генерируешь короткие вопросы для самопроверки. Только JSON массив строк, без пояснений, без markdown.', qPrompt);
...
async function sendMatChat() {
  ...
  try {
    ...
    if (ctx?.file && ctx.file.type.startsWith('image/')) {
      ...
      const transcript = ctx.transcript ? `\n\nСубтитры видео (используй как основу):\n${ctx.transcript.slice(0, 6000)}` : '';
      const system = `Ты AI-ассистент. Пользователь смотрит урок по теме "${topicTitle}".

${videoInfo}${transcript}

ПРАВИЛА ОТВЕТА:
— Отвечай КОРОТКО и по делу — 2–4 предложения максимум, если вопрос простой
— Если вопрос требует объяснения формулы или шагов — отвечай чуть подробнее, но без воды
— Опирайся на содержание видео если оно есть в субтитрах
— Никаких вступлений: "Конечно!", "Отличный вопрос!", "Давайте разберём"
— Формулы пиши в LaTeX: $формула$
— Только русский язык`;
